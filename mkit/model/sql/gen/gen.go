...
import (
	...
	"github.com/uncleyeung/devtools/mkit/config"
	"github.com/uncleyeung/devtools/mkit/model/sql/model"
	"github.com/uncleyeung/devtools/mkit/model/sql/parser"
	"github.com/uncleyeung/devtools/mkit/model/sql/template"
	modelutil "github.com/uncleyeung/devtools/mkit/model/sql/util"
)
...
func (g *defaultGenerator) StartFromDDL(filename string, withCache bool, database string, gitlab string, repo string) error {
	modelList, err := g.genFromDDL(filename, withCache, database, gitlab, repo)
	if err != nil {
		return err
	}
...
func (g *defaultGenerator) createFile(modelList map[string]string) error {
	...
	for tableName, code := range modelList {
		...
		modelFilename := fmt.Sprintf("%s_dao", tn.Source())
...
func (g *defaultGenerator) genFromDDL(filename string, withCache bool, database string, gitlab string, repo string) (map[string]string, error) {
	m := make(map[string]string)
	tables, err := parser.Parse(filename, database, gitlab, repo)
	if err != nil {
		return nil, err
	}
...
func (g *defaultGenerator) genModel(in parser.Table, withCache bool) (string, error) {
	...
	var list []string
	list = append(list, insertCodeMethod)
	if findOneCodeMethod != "" {
		list = append(list, findOneCodeMethod)
	}
	if updateCodeMethod != "" {
		list = append(list, updateCodeMethod)
	}
	if deleteCodeMethod != "" {
		list = append(list, deleteCodeMethod)
	}
