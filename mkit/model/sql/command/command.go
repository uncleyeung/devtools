...
import (
	...
	"github.com/uncleyeung/devtools/mkit/model/sql/gen"
...
func MySqlDataSource(ctx *cli.Context) error {
	...
	return fromMysqlDataSource(url, pattern, dir, cfg, cache, idea, gitlab, repo)
}
...
func fromMysqlDataSource(url, pattern, dir string, cfg *config.Config, cache, idea bool, gitlab, repo string) error {
	log := console.NewConsole(idea)
	if len(url) == 0 {
