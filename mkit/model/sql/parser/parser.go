...
import (
	...
	"github.com/uncleyeung/devtools/mkit/model/sql/converter"
	"github.com/uncleyeung/devtools/mkit/model/sql/model"
	"github.com/uncleyeung/devtools/mkit/model/sql/util"
...
func ConvertDataType(table *model.Table) (*Table, error) {
	...
	seqInIndex := 0
	primaries := make([]Primary, 0)
	for _, value := range table.PrimaryKey {
		if value == nil {
			continue
		}
		isPrimaryDefaultNull := value.ColumnDefault == nil && value.IsNullAble == "YES"
		primaryDataType, err := converter.ConvertStringDataType(value.DataType, isPrimaryDefaultNull)
		if err != nil {
			return nil, err
		}

		if value.Index != nil {
			seqInIndex = value.Index.SeqInIndex
		}
		primaries = append(primaries, Primary{
			Field: Field{
				Name:            stringx.From(value.Name),
				DataType:        primaryDataType,
				Comment:         value.Comment,
				SeqInIndex:      seqInIndex,
				OrdinalPosition: value.OrdinalPosition,
			},
			AutoIncrement: strings.Contains(value.Extra, "auto_increment"),
		})
	}

	if len(primaries) > 0 {
		reply.PrimaryKey = primaries[0]
	}
	...
	for indexName, each := range table.UniqueIndex {
		...
		if len(each) == 1 {
			one := each[0]
			for _, valle := range table.PrimaryKey {
				if one.Name == valle.Name {
	...
	reply.Gitlab = table.Gitlab
	reply.Repo = table.Repo
	return &reply, nil
}
